  ...
  const OverviewView = () => {
    ...
    return (
      <VStack spacing={8} align="stretch">
        ...
        {/* System Status Widget (New Feature) */}
        <Box
          p={6}
          borderRadius="2xl"
          bg="rgba(0, 0, 0, 0.4)"
          ...
          backdropFilter="blur(10px)"
        >
          <Heading size="md" mb={4} color="gray.300" display="flex" alignItems="center" gap={2}>
            <Icon as={Server} color="cyan.400" /> System Status
          </Heading>
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6}>
            <HStack spacing={4} p={3} bg="whiteAlpha.50" borderRadius="lg" border="1px solid" borderColor="whiteAlpha.100">
              <Box p={2} bg="green.500" borderRadius="full" boxShadow="0 0 10px green">
                <Icon as={Database} size={20} color="white" />
              </Box>
              <Box>
                <Text fontSize="sm" color="gray.400">Database</Text>
                <Text fontSize="md" fontWeight="bold" color="green.400">Operational</Text>
              </Box>
            </HStack>
            <HStack spacing={4} p={3} bg="whiteAlpha.50" borderRadius="lg" border="1px solid" borderColor="whiteAlpha.100">
              <Box p={2} bg="blue.500" borderRadius="full" boxShadow="0 0 10px blue">
                <Icon as={Cpu} size={20} color="white" />
              </Box>
              <Box>
                <Text fontSize="sm" color="gray.400">AI Microservice</Text>
                <Text fontSize="md" fontWeight="bold" color="blue.400">Online (v2.1)</Text>
              </Box>
            </HStack>
            <HStack spacing={4} p={3} bg="whiteAlpha.50" borderRadius="lg" border="1px solid" borderColor="whiteAlpha.100">
              <Box p={2} bg="purple.500" borderRadius="full" boxShadow="0 0 10px purple">
                <Icon as={HardDrive} size={20} color="white" />
              </Box>
              <Box>
                <Text fontSize="sm" color="gray.400">Storage</Text>
                <Text fontSize="md" fontWeight="bold" color="purple.400">85% Free</Text>
              </Box>
            </HStack>
          </SimpleGrid>
        </Box>
        ...
        <Box mb={8}>
          <Heading size="md" mb={4} color="gray.300" display="flex" alignItems="center" gap={2}>
            <Icon as={AlertTriangle} color="red.400" /> Attention Needed
          </Heading>
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6}>
            {submissions.filter(s => (s.audit_security_score && s.audit_security_score < 70) || (s.audit_quality_score && s.audit_quality_score < 60)).slice(0, 3).map(sub => (
              <Box key={sub.id} p={4} bg="rgba(245, 101, 101, 0.1)" borderRadius="xl" border="1px solid" borderColor="red.500">
                <HStack justify="space-between" mb={2}>
                  <Text fontWeight="bold" color="red.300" fontSize="sm">Quality Alert</Text>
                  <Badge colorScheme="red">Action Req</Badge>
                </HStack>
                <Heading size="sm" color="white" mb={1}>{sub.title}</Heading>
                <Text fontSize="xs" color="gray.400">Audit Score Low: {sub.audit_quality_score}/100</Text>
              </Box>
            ))}
            {unappointedSubmissions.length > 0 && (
              <Box p={4} bg="rgba(236, 201, 75, 0.1)" borderRadius="xl" border="1px solid" borderColor="yellow.500">
                <HStack justify="space-between" mb={2}>
                  <Text fontWeight="bold" color="yellow.300" fontSize="sm">New Projects</Text>
                  <Badge colorScheme="yellow">Pending</Badge>
                </HStack>
                <Heading size="sm" color="white" mb={1}>{unappointedSubmissions.length} Unassigned</Heading>
                <Text fontSize="xs" color="gray.400">Students waiting for appointment.</Text>
              </Box>
            )}
            {/* Fallback if no specific attention needed */}
            {submissions.every(s => !s.audit_security_score || s.audit_security_score >= 70) && unappointedSubmissions.length === 0 && (
              <Box p={4} bg="rgba(72, 187, 120, 0.1)" borderRadius="xl" border="1px solid" borderColor="green.500" gridColumn="span 3">
                <HStack>
                  <Icon as={CheckCircle} color="green.400" />
                  <Text color="green.200">System Healthy - No immediate actions required.</Text>
                </HStack>
              </Box>
            )}
          </SimpleGrid>
        ...
        <SimpleGrid columns={{ base: 1, lg: 3 }} spacing={8}>
          ...
          <Box gridColumn={{ lg: "span 2" }}>
            ...
            >
              <Heading size="md" mb={6} color="gray.300" display="flex" alignItems="center" gap={2}>
                <Icon as={TrendingUp} color="purple.400" /> Submission Volume
              </Heading>
              <HStack align="flex-end" justify="space-between" h="100px" px={2}>
                {[35, 50, 25, 60, 45, 80, 55].map((h, i) => (
                  <VStack key={i} spacing={2}>
                    <Box
                      w="8px"
                      h={`${h}px`}
                      bgGradient="linear(to-t, purple.600, blue.400)"
                      borderRadius="full"
                    />
                    <Text fontSize="xs" color="gray.500">{['M', 'T', 'W', 'T', 'F', 'S', 'S'][i]}</Text>
                  </VStack>
                ))}
              </HStack>
            </Box>
        ...
        <VStack spacing={8} align="start" h="full">
          ...
          <VStack spacing={2} w="full" align="stretch">
            ...
            <Button
              ...
              colorScheme={activeView === 'unappointed_ongoing' ? 'cyan' : 'gray'}
              justifyContent="flex-start"
